...
def main(args):
    ...
    # Given a video path
    fn = args.video_fp.split('/')[-1] # 214.avi
    ...
    # run
    dense_flag = args.opt in ('3d',) # True for 3D
    pre_ver = None
    param_f = []
    frame_idx = []
    for i, frame in tqdm(enumerate(reader)):
        ...
        # save features
        frame_idx.append(i)
        image_ids = np.array(frame_idx)
        param_f.append(param_lst)
        param_np = np.array(param_f)
    ...
    param_np = param_np.squeeze(1)
    np.savez(f'examples/results/param/{fn.replace(suffix, "")}.npz', image_inds=image_ids, tddfa=param_np)
